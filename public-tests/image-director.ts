...
const pictured = applyParsedScene(initial, importantDialogue, cartridge, '听媛夕说明')
const expressionImage = latestImage(pictured)
equal(expressionImage?.id, 'image-1', 'substantive important-character dialogue guarantees an image')
...
const environmentProposal = applyParsedScene(initial, importantDialogue, cartridge, '听媛夕说明', 'empty forest lamps in fog, wide environment-only shot', 'environment')
...
equal(latestImage(applyParsedScene(initial, followedByMinorLine, cartridge, '继续听'))?.data?.identityCharacterId, 'mira-voss', 'a later minor line cannot hide the important speaker')

const shortAdministrativeLine = parseStoryProtocol(`[媛夕] [main] [平静]: "知道了。"
[character_update: character_id="mira-voss" character="媛夕" role="28 岁 · 地方植物研究员"]
[choices: "继续赶路"|"查看地图"|"稍作休息"]`, 'zh')
equal(latestImage(applyParsedScene(initial, shortAdministrativeLine, cartridge, '点头'))?.id, 'image-0', 'short administrative acknowledgement does not force a portrait')
