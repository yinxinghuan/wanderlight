    ...
    rules: [
      {
        ...
        match: [s('找一份短工', 'look for a short job'), s('做短工', 'take a local shift'), s('继续干活', 'keep working'), s('帮忙干活', 'help with the work')],
        ...
        rejectionChoices: [s('花四枚旅费吃一顿热饭', 'Spend four coin on a hot meal'), s('找个安静角落休息', 'Rest somewhere quiet'), s('问熟人能不能帮忙', 'Ask someone you know for help')],
      },
      {
        id: 'hot-meal', intent: s('吃一顿热饭', 'eat a hot meal'),
        match: [s('吃一顿热饭', 'eat a hot meal'), s('吃点东西', 'get something to eat'), s('买一顿饭', 'buy a meal')],
        requirements: [{ type: 'stat', id: 'coin', min: 4, reason: s('你还差几枚旅费，摊主没有答应赊账。', 'You are a few coin short, and the vendor will not open a tab.') }],
      ...
      },
      {
        ...
        requirements: [{ type: 'stat', id: 'coin', min: 10, reason: s('房费是十枚旅费，你现在付不起。', 'The room costs ten coin, which you cannot afford yet.') }],
      ...
      ...[
        ...
        requirements: [
          ...
          { type: 'stat' as const, id: 'coin', min: 3, reason: s('普通车票需要三枚旅费。你可以先接短工，或找乘务员谈谈。', 'A regular ticket costs three coin. Take a short job or speak with the steward first.') },
...
function worldMap(locale: Locale): StoryCartridge['initialMap'] {
  ...
  return [
    ...
    },
    {
      ...
      facts: [s('藤架修补和田野记录需要短工', 'Trellis repair and field surveys need temporary help'), s('晚餐长桌与试饮不要求发展亲密关系', 'Supper and tasting invitations carry no romantic obligation'), s('田舍有十枚旅费的客房', 'The field house rents rooms for ten coin')],
    },
    {
    ...
    },
    {
      ...
      facts: [s('夜间工坊会发布修理与记录工作', 'Night workshops post repair and recording jobs'), s('开放讲座和公共工作台允许访客参加', 'Open lectures and shared benches welcome adult visitors'), s('空余客房每晚十枚旅费', 'Available guest rooms cost ten coin per night')],
    },
  ]
}
...
function make(locale: Locale): StoryCartridge {
  ...
  return {
    ...
    audioTheme: { material: 'wayfarer', bpm: 68, rootHz: 146.83, scale: [0, 2, 5, 7, 9], levels: { music: 0.11, ambient: 0.1, sfx: 0.16, master: 0.72 }, tension: [{ statId: 'energy', direction: 'low', weight: 0.6 }, { statId: 'coin', direction: 'low', weight: 0.25 }, { statId: 'renown', direction: 'low', weight: 0.15 }] },
    itemImageDirection: 'quiet travel-object study on worn indigo cloth and pale station stone, object only, no people, no text',
    ...
    imageDirector: { maxQuietTurns: 4, softCooldownTurns: 2, guaranteedTriggers: ['new-location', 'relationship-change'], softTriggers: ['party-change', 'objective-change', 'chapter-checkpoint'] },
    ...
    initialFacts: { all_intimate_characters_adult: true, dynamic_identity_trial: true, moonline_stamps_used: 0, world_day: 1, jobs_completed: 0, meals_eaten: 0, nights_slept: 0, carriage_rests: 0 },
    statDefinitions: [{ id: 'energy', label: s('精力', 'Energy'), min: 0, max: 100, initial: 72, display: 'bar', inverse: true, warningAt: 28, dangerAt: 8, maxDelta: 24 }, { id: 'coin', label: s('旅费', 'Coin'), min: 0, max: 120, initial: 18, display: 'number', inverse: true, warningAt: 8, dangerAt: 0, maxDelta: 30 }, { id: 'renown', label: s('风闻', 'Standing'), min: -40, max: 100, initial: 4, display: 'bar', inverse: true, warningAt: -10, dangerAt: -30, maxDelta: 18 }],
    domainRules: domainRules(locale),
    drawerLabels: { party: s('同行', 'People'), map: s('路线', 'Routes'), inventory: s('行囊', 'Pack'), log: s('旅记', 'Journal') },
    ...
    initialInventory: [{ id: 'moonline-passbook', label: s('月线通行册', 'Moonline Passbook'), count: 1, rarity: 'rare', detail: s('蓝灰布封面的小册，夹着三枚无字银色印章。', 'A blue-gray cloth passbook holding three unlettered silver stamps.'), effect: s('每枚印章可为已发现路线换一次停运后的夜班席位。', 'Each stamp secures one after-hours seat on a discovered route.'), lore: s('灯湾月线给临时工作人员的旧式凭证。', 'An old credential issued to temporary Moonline workers.'), metrics: [{ id: 'stamps', label: s('剩余印章', 'Stamps remaining'), value: '3 / 3' }], imagePrompt: 'single blue-gray cloth railway passbook with three unlettered silver stamp tokens, worn indigo cloth and pale station stone, object only, no hands, no text, square' }],
    demoTurns: [
      { match: zh ? ['种荚', '短发', '帮'] : ['seed', 'short-haired', 'help'], content: miraDebut, imagePrompt: 'Lantern Quay platform at blue hour, medium environmental shot of one adult field botanist securing luminous seed cases, short asymmetrical deep-brown bob, narrow copper hairpin above right ear, sage jacket and copper seed-pod pendant visible, player only as out-of-focus hand at frame edge, no text, no UI, 4:3', imageSubject: 'others', imageCharacterId: 'mira-voss' },
