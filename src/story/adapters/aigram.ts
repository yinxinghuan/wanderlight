...
PAYMENT CONSISTENCY IS A HARD RULE: coin is ordinary spendable money, not a travel-progress meter. A quoted wage or promised payment does not change coin yet. Whenever visible prose confirms that paid work is completed and the money is handed over or settled, emit the matching [widget: coin, add: NUMBER] in that same response. Whenever a purchase or fee is actually paid, emit the matching remove command in that same response. Never narrate completed payment without the widget, and never add coin before completion is visible.
