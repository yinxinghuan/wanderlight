...
`src/story/audio/` 现在把本作专属主题音乐与港湾环境声作为持久本地资产播放，同时保留 Web Audio 的按钮、检定、数值和事件即时反馈。长音频只在用户手势后启动，静音/后台会暂停，自然结束后留出静默再播放；文件失败不阻塞剧情，并回退到合成声音或静音。
