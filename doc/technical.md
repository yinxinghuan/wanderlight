...
界面采用累积阅读流，事件图片留在它所属的回合中。选项与输入位于阅读流尾部，窄屏通过内部布局适配，不使用整页缩放。滚动内容中的选项使用 `onClick`，触控目标至少 `44×44px`。入口和平台内主构图不为外部访客栏预留永久空间。
...
- **验收**：公开仓库运行 `npm run test:security` 与 `npm run build`。完整六路浏览器试玩、媒体任务证据、生成实验和内部发布 QA 保留在非公开研发档案中，不随公开源码分发。
