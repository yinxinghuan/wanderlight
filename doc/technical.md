...
动态有偿工作使用 `[job]` 协议：`offer` 固化稳定工作 ID、雇主、工作内容和明确工资，`settle` 只能结算未完成合同，并由 reducer 直接按记录工资增加钱币和 `jobs_completed`；同回合额外的 coin widget 会被拒绝且 reducer 也做去重防御。`paymentConsistency.ts` 在任何生成正文写入存档前机械核对钱币、铜板、铜币、硬币等报价、付款、消费和合同金额；首次失败会要求当前适配器完整重写同一回合，第二次仍失败则不提交。已知的“种荚冷藏后只写几枚铜板”与“夜市酱料整理后只写几枚铜币”旧档只在精确命中各自坏剧情且缺少迁移事实时补原定 `8` 枚一次，不要求当前余额仍为初始值，因此后续消费不会阻止补账；独立迁移事实防止重复。

每个非 demo 生成回合还必须输出唯一 `[scene_location]`；正文抵达新地点必须同回合输出 `[map_update]`，明确接受或被交付一项新任务时必须输出 `[state]`，场景图必须用 `[image_location]` 声明与当前场景相同的地点。`turnConsistency.ts` 在提交前联合检查有效地点、任务、最终选项和图片提示；缺失的纯协议地点标签会从权威存档补齐，缺少地点绑定的图片提议会被丢弃并交由本地图片导演重建，只有可见正文确实建立新任务时才补目标，普通玩家动作不会再被误写成长期目标。真正的旧地点选项、旧地点图片或状态冲突仍会触发一次完整重写；重写仍不一致时 `applyConsistencyRecovery()` 拒绝不可靠内容、保持地点/数值/物品/关系不变，用世界内文字解释刚才的行动为什么暂时停在当前地点，并把原行动保留为第一项。其余两项不再是伪装成可玩路线的模型请求：`resolveConsistencyRecoverySelection()` 识别“查看当前能做的事”和“放弃原计划”，`applyConsistencyRecoverySelection()` 在本地写入确定性解释、生成去重的当前可执行行动，并立即退出恢复态。旧版本已经保存的泛化恢复回合会在载入时同步迁移正文、文章选项和底部选项；已形成“确认路线 → 再次确认”的嵌套恢复链会回溯到最早的真实玩家行动，不再重复自己；若旧目标正是被普通行动污染的值，则对齐到最新未完成意图。对已经出现“雾杉林巡逻正文、灯湾码头按钮和旧图”的旧存档，只在精确命中该组正文与旧选项时迁移到雾杉林、恢复三个巡逻行动并排队重生成该回合图片，迁移事实确保只执行一次。

`canonicalizeTurnMetadata()` 还会在回合提交前逐项处理选择：删除不能在新场景中直接行动的旧地点选项，再用 `filterGroundedChoices()` 删除未被可见正文或权威存档建立的人物、物品、机构和目标。过滤是逐项而不是整组否决；剩下 `1–5` 项都可直接提交和渲染。只有零个可靠选项时才进入本地恢复，`createRecoveryChoices()` 也只根据当前已存在的地点、目标和同行者生成出口，不再用“换一种方式”等占位文案补数。
