...
`engine/executeTurn.ts` 是与 React/DOM/媒体/存储解耦的服务端回合权威边界，并保留活跃危险 deflection、领域事务、作者回合、proposal 校验与 reducer 的原顺序。`_qa/server-turn-pipeline.ts` 验证模型旁路、原子提交、输入不变性和危险线程保护。
